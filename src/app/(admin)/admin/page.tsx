...
import { Building, Users, CalendarDays, Loader2, Plus, ChevronRight, AlertTriangle } from "lucide-react";
...
export default function AdminDashboard() {
    ...
    const overviewStats = [
        { title: "Propriétés Totales", value: stats?.totalProperties || 0, icon: Building, color: "text-blue-500" },
        { title: "Propriétés Vacantes", value: stats?.vacantProperties || 0, icon: Building, color: "text-green-500" },
        { title: "Occupations Actives", value: stats?.activeOccupations || 0, icon: Users, color: "text-primary" },
        { title: "Échéances (7j)", value: stats?.upcomingExpirations || 0, icon: AlertTriangle, color: "text-orange-500" },
    ];
                    ...
                    <section>
                        <Card className="border-none shadow-sm rounded-2xl overflow-hidden">
                            <CardHeader className="p-8 pb-0">
                                <CardTitle className="text-2xl font-bold text-slate-800">Calendrier des Occupations</CardTitle>
                                <p className="text-slate-500">Visualisez et gérez toutes les dates d&apos;occupation de vos biens</p>
                            </CardHeader>
                            <CardContent className="p-8">
                                <EnhancedCalendar occupations={stats?.allActiveOccupations || []} />
                            </CardContent>
                        </Card>
                    </section>
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                        ...
                        <Card className="border-none shadow-sm rounded-2xl overflow-hidden transition-all hover:shadow-md">
                            <CardHeader className="flex flex-row items-center justify-between p-8 pb-4">
                                <div>
                                    <CardTitle className="text-xl font-bold text-slate-800">Dernières Occupations</CardTitle>
                                    <p className="text-sm text-slate-500 mt-1">Activité récente</p>
                                </div>
                            </CardHeader>
                            <CardContent className="px-6 pb-8">
                                <div className="space-y-3">
                                    {stats?.recentOccupations?.length === 0 ? (
                                        <div className="text-center py-12 bg-slate-50 rounded-xl border border-dashed border-slate-200">
                                            <p className="text-slate-500 font-medium">Aucune occupation ajoutée.</p>
                                        </div>
                                    ) : (
                                        stats?.recentOccupations?.slice(0, 4).map((occ: any) => (
                                            <div key={occ._id} className="flex items-center justify-between p-4 bg-slate-50/50 hover:bg-slate-50 rounded-xl transition-all border border-transparent hover:border-slate-100 group">
                                                <div className="flex items-center gap-3">
                                                    <div className="h-10 w-10 rounded-full bg-white shadow-sm flex items-center justify-center text-primary font-bold text-sm shrink-0">
                                                        {occ.tenantName.charAt(0)}
                                                    </div>
                                                    <div className="flex flex-col min-w-0">
                                                        <span className="font-bold text-slate-800 text-sm truncate">{occ.tenantName}</span>
                                                        <span className="text-[11px] text-slate-400 font-medium truncate">
                                                            {occ.propertyId?.title || "Bien supprimé"}
                                                        </span>
                                                    </div>
                                                </div>
                                                <div className="text-right shrink-0">
                                                    <div className="text-xs text-slate-500">Du {format(new Date(occ.startDate), "dd/MM/yy")}</div>
                                                    <div className="text-xs text-slate-500">Au {format(new Date(occ.endDate), "dd/MM/yy")}</div>
                                                </div>
                                            </div>
                                        ))
                                    )}
                        ...
                        <Card className="border-none shadow-sm rounded-2xl overflow-hidden flex flex-col">
                            <CardHeader className="p-8 pb-4">
                                <CardTitle className="text-xl font-bold text-slate-800">Vue Rapide (Mois)</CardTitle>
                            </CardHeader>
                            <CardContent className="p-8 pt-0 flex justify-center flex-1 items-center">
                                <div className="scale-90 origin-center">
                                    <AdminCalendar occupations={stats?.allActiveOccupations || []} />
                                </div>
                            </CardContent>
                        </Card>
                    </div>
